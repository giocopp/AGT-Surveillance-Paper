...
between surveillance authorities and those subjected to monitoring. In fact, the latters might choose to adapt their behaviours to defy such systems, and in turn, the surveillance authorities may be able to learn from the adapted behavious and change their strategy accordingly. This paper asks: How do targets of algorithmic surveillance adapt their behavior? Does this strategic interaction push systems toward harsher policies or systematic errors? And crucially, are such errors tolerated 
behavior? Does this strategic interaction push systems toward harsher policies or systematic errors? And crucially, are such errors tolerated  or even instrumentalized because they serve broader goals of population control? 
...
formal model. Section 5 analyzes equilibrium behavior and comparative statics. Section 6 discusses policy implications and limitations. Section 7 identifies a venue for this case to be modeled as a signaling game, providing durection for further research. 
...
within Israeli society and intensify control over Palestinians. Additionally, studies suggest that algorithmic `errors', like false positives and 
...
control. A piece of investigative journalism by +972 Magazine explains that in the context of the search and elimination of targets, the Israely army has dramatically expanded their definition of "human target" since October 7, when Hamas-led militants launched a deadly assault on southern Israeli communities (\citeyear{Abraham2024Lavender}). From being a term used to designate senior military operative, which numbered only a few dozen people that could be inspected by intelligence personell and located individually, under “Operation Iron Swords,” the army decided to designate all operatives of Hamas’ military wing as human targets, regardless of their rank or military importance. Being now the number of the targets in the thousands, the IDF recurred to the use of a new AI system, called Lavander, to add people to their 'kill list'. The approval to automatically adopt Lavender’s kill lists was only granted about two weeks into the war, after intelligence manually checked its accurate detection out of a random sample of several hundred targets. Even if Lavander is used in a different context than Blue wolf, this shows that to hunt down security threats, the AI that the army use is highly prone to errors and misclassifications, and gives reasons to believe that the AI used for population control might have similar faults. Additionally, detecting many real security threats would reinforce the counter-terrorism frame that benefits the political goals of Israel and its allies, which suggests that a miscassification might not only be unaccounted for, but even desirable. The literature gives reasons to believe that the IDF misclassifies civilians as real security threats, and might have political interests to do so. The game-theoretic framework developed below formalizes this intuition.
...
A possible expansion of this model could be to reinterpret the Hebron setting as a signaling game, specifically as an adaptation of the classic Beer--Quiche game. In the traditional Beer--Quiche game, individuals of different types choose between two observable signals (beer or quiche) to convey their type to a receiver who then takes an action based on the observed signal. The sender's type is private information, and the receiver must infer the sender's type from the chosen signal. Within this framework, we can reinterpret the Hebron setting as a signaling environment in which Palestinians are the senders and IDF soldiers operating Blue Wolf are the receivers. Each Palestinian has a private type $\theta \in \{T, N\}$, where $T$ denotes a real security concern, hence a potential terrorist, and $N$ a civilian. Crucially, unlike in the classic model, both types strictly prefer to be classified as ``civilian'' rather than ``terrorist,'' because any terrorist classification leads to detention, harassment, or long-term consequences in the database. Observable signals include participation in protests, dense social networks, movement patterns, or other behaviors that are algorithmically encoded as risk factors. Soldiers, aided by Blue Wolf, map these signals into a binary action: classify and treat the individual as terrorist (red/yellow) or civilian (green).

In a standard Beer--Quiche setup, equilibrium often hinges on one type having a differential taste for a costly signal (e.g., tough types liking beer more), which can sustain a separating equilibrium. Here, however, protest or politically salient behavior is costly for both types once it is surveilled, because the algorithm increases the probability of terrorist classification regardless of $\theta$. This pushes the game toward pooling on low-risk signals, with both $T$ and $N$ types strategically avoiding protest or visible dissent---precisely the self-disciplining adaptation highlighted by our model. From the soldiers' perspective, the algorithm acts like an imprecise decoding rule that sometimes classifies $N$ as $T$. When such false positives are politically instrumental (high $\alpha$), the receiver is effectively rewarded for treating many signals as evidence of terrorism, which as in the model we tackled in the previous sections, shrinks the range of behaviors that can safely function as ``civilian'' signals and reinforcing a pooling equilibrium of widespread self-silencing among civilians.
...
\newblock +972 Magazine, April 3, 2024. Retrieved from \url{https://www.972mag.com/lavender-ai-israeli-army-gaza/}
