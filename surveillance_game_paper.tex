...
\begin{titlepage}
    ...
    technological advances in surveillance create feedback loops that may systematically push toward harsher control policies. 
    We introduce an interpretation suggesting that algorithmic errors and false positives, rather than being bugs to be eliminated, 
    are tolerated or even strategically advantageous that serve the underlying political economy of population control. The paper 
    contributes to debates on algorithmic governance by demonstrating how formal game theory can illuminate the political logic of 
...
between surveillance authorities and those subjected to monitoring. In fact, the latters might choose to adapt their behaviours to defy such 
systems, and in turn, the surveillance authorities may be able to learn from adaptiation and change their strategy accordingly. 
...
occupation, defining the system as ``the Facebook of Palestininans'', and Hebron as a ``Smart City'', unmasking their dual logic of surveillance
\citep{dwoskin2021israel}. According to experts, Israel’s use of surveillance and facial recognition appear to be among the most advanced 
...
\citep{goodfriend2023algorithmic}. 
...
model of digital authoritarianism and preventive repression, we extend their analysis to incorporate the specific features of algorithmic 
...
control regardless of actual security outcomes. The paper proceeds as follows. Section 2 situates the analysis within literature on algorithmic 
governance, surveillance studies, and game-theoretic approaches to repression. Section 3 presents the empirical context of Blue Wolf in Hebron. 
Section 4 develops the formal model. Section 5 analyzes equilibrium behavior and comparative statics. Section 6 discusses policy implications 
and limitations. Section 7 identifies a venue for this case to be modeled as a signaling game, providing durection for further research. 
...
It is important to note that, even if Israel is commonly classified as a democracy in terms of electoral institutions, 
a substantial literature argues that its governance toward Palestinians is characterized by stratified citizenship and coercive control 
practices consistent with illiberal or authoritarian logics in that domain 
\citep{dayan2022dualstate,gidron2023crisis,jamal2007hollow,smooha2002ethnic,rouhana1997identities,peled2008evolution}.
That is why we will consider Israel as an authoritarian state in this setting.
...
Israeli army has dramatically expanded their definition of "human target" since October 7th, when Hamas-led militants launched a deadly assault 
on southern Israeli communities \citep{abraham2024lavender}. From being a term used to designate senior military operative, under “Operation 
Iron Swords,” the army decided to designate all operatives of Hamas’ military wing as human targets, regardless of their rank or military 
importance. Being now the number of the targets in the thousands, the IDF recurred to the use of a new artificial intelligence system, called 
Lavander, to add people to their 'kill list'. The approval to automatically adopt Lavender’s kill lists was only granted about two weeks into 
the war, even if the program is relatively highly prone to errors and misclassifications \citep{abraham2024lavender}. This precendent gives reasons to 
...
miscassification might not only be unaccounted for, but even desirable. The literature gives reasons to believe that the IDF misclassifies 
civilians as real security threats, and might have political interests to do so. The game-theoretic framework developed below formalizes this 
intuition.
...
Our model has also some evident limitations. The first is that it is static, while the Blue Wolf context and the broader literature emphasize dynamic feedback 
...
type $\theta \in \{T, N\}$, where $T$ denotes a real security concern, hence a potential terrorist, and $N$ a civilian. Crucially, unlike in 
the classic model, both types strictly prefer to be classified as ``civilian'' rather than ``terrorist,'' because any terrorist classification 
leads to detention, harassment, or long-term consequences in the database. Observable signals include participation in protests, dense social 
networks, movement patterns, or other behaviors that are algorithmically encoded as risk factors. Soldiers, aided by Blue Wolf, map these 
signals into a binary action: classify and treat the individual as terrorist (red/yellow) or civilian (green).
...
is surveilled, because the algorithm increases the probability of terrorist classification regardless of $\theta$. This pushes the game toward 
pooling on low-risk signals, with both $T$ and $N$ types strategically avoiding protest or visible dissent. From the soldiers' perspective, 
...
the previous sections, shrinks the range of behaviors that can safely function as ``civilian'' signals and reinforcing a pooling equilibrium 
